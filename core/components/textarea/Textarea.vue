<template>
  <div>
    <lv-input type="text" :editable="false" ref="mainInput" v-bind="$attrs" :class="{ '--not-empty': filled }" :value="modelValue" @clear="handleClear()">
      <textarea :maxlength="maxLength" class="lv-textarea light-scrollbar" v-bind="$attrs" @input="updateModel" :value="modelValue" ref="textarea" :class="{ '--resize-vertical': resize === 'vertical' }" />
    </lv-input>
